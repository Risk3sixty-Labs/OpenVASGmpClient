...
export default function Tasks() {
  return {
    ...
    async create_task({
      name, // required
      comment,
      config, // required, uuid
      target, // required, uuid
      scanner //required, uuid
    }) {
      await this.connect()

      let json = { create_task: [
        { name: 'name', text: name },
        { name: 'comment', text: comment },
        { name: 'config', attrs: { id: config }},
        { name: 'target', attrs: { id: target }},
        { name: 'scanner', attrs: { id: scanner }}
      ]}
    ...
    async get_tasks(uuid=null) {
      ...
      let json = [{ name: 'get_tasks' }]
      if (uuid)
        json[0] = { ...json[0], attrs: { task_id: uuid }}
    ...
    async get_task_results(taskUuid, resultId=null) {
      ...
      // Give root element attributes
      // https://github.com/soldair/node-jsontoxml/issues/26
      let json = [{ name: 'get_results', attrs: {}}]
      if (resultId)
        json[0].attrs.result_id = resultId
      else if (taskUuid)
        json[0].attrs.filter = `task_id=${taskUuid}`
